...
def main():
    ...
    col, dummy = ascii_dat_read("MeerKAT_pads.txt", delim=",",
                                       doFloatCols=[1, 2])
    padDict = {}
    pad = 0
    ...
    col, dummy = ascii_dat_read("MeerKAT_configs.txt", delim=" ")
    for confName, padStr in zip(col[1], col[2]):
        padLst = [int(x) for x in padStr.split(",")]

        outFileName = "MeerKAT_%s.config" % confName
